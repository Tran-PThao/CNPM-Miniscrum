...
const EditProject = () => {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        const res = await axios.get(`http://${window.location.hostname}:5001/api/project/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.patch(`http://${window.location.hostname}:5001/api/project/${id}`, formData, {
